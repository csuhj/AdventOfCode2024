...
bool CanEquationBeValid(long testValue, long[] operands)
{
    ...
    long[] tailOperandsConcatenated = operands.Skip(1).ToArray();
    tailOperandsConcatenated[0] = long.Parse($"{headOperand}{tailOperandsConcatenated[0]}");

    return CanEquationBeValid(testValue, tailOperandsMultiplied) || CanEquationBeValid(testValue, tailOperandsAdded) || CanEquationBeValid(testValue, tailOperandsConcatenated);
}